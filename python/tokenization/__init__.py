...
class BPEEncoder:
    ...
    def gtViewMerges(self, builder):
        ...
        clist.column(
            "Pair",
            lambda each: [
                chr(char.byte) if char.byte < 256 else char.byte for char in each[0]
            ],
        )
        return clist
    ...
    def train(self, text, vocab_size, verbose=False):
        ...
        ids = [
            EncoderByte(byte) for byte in text_bytes
    ...
    def merge(self, ids, pair, merge):
        ...
        while i < len(ids):
            if (
                i < len(ids) - 1
                and ids[i].byte == pair[0].byte
                and ids[i + 1].byte == pair[1].byte
            ):
