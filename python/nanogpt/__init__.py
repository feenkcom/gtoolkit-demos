...
class BigramLanguageModel(nn.Module):
    def __init__(
        self, vocab_size, block_size, device, n_embd, n_head, n_layer, dropout
    ):
        super().__init__()
...
def train(data, vocab_size, hyperparameters=None):
    ...
    model = m.to(hyperparameters["device"])
    ...
    return TrainingContext(model=model, hyperparameters=hyperparameters)
...
def generate_tokens(training_context, max_tokens, context=None):
    ...
    return training_context.model.generate(context, max_new_tokens=max_tokens)[
        0
    ].tolist()
